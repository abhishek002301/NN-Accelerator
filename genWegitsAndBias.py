            ...
            for weight in range(0, len(myWeights[layer][neuron])):
                if 'e' in str(myWeights[layer][neuron][weight]):
                    ...
                    if myWeights[layer][neuron][weight] > 2**(weightIntWidth-1):
                        myWeights[layer][neuron][weight] = 2**(
                            weightIntWidth-1)-2**(-weightFracWidth)
                    elif myWeights[layer][neuron][weight] < -2**(weightIntWidth-1):
                        myWeights[layer][neuron][weight] = - \
                            2**(weightIntWidth-1)
                    wInDec = DtoB(myWeights[layer][neuron]
                                  [weight], dataWidth, weightFracWidth)
                    p = bin(wInDec)[2:]
                f.write(p+'\n')
...
if __name__ == "__main__":
    ...
    maxVal = max(output)
    minVal = min(output)
    minBits = max(math.ceil(math.log2(abs(int(maxVal))+1)),
                  math.ceil(math.log2(abs(int(minVal))+1)))+1  # One additional bit for sign
    ...
    src = './w_b'
    trg = './src/fpga/rtl'
    trg1 = './myProject1/myProject1.sim/sim_1/behav/xsim'
    files = os.listdir(src)
    for fname in files:
        shutil.copy2(os.path.join(src, fname), trg)
        shutil.copy2(os.path.join(src, fname), trg1)
